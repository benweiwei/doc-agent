...
- 无文档直接输入需求：未打开任何文档时提交指令，自动以指令内容新建文档并进入 AI 生成流程（标题自动去重，输入框占位符与进度提示同步更新）
- Agent 支持从零撰写新文档：目标文档不存在时按空文档处理，提示词约束模型写入指定文档 ID；模型擅自改名时后端自动认领其写入结果

### 修复

- 兼容只允许默认 temperature 的模型（如 Moonshot kimi-k3）：请求被 400 拒绝后自动去掉 temperature 重试，并记住该行为避免重复浪费一次往返
- 客户端断开 WebSocket 后 Agent 循环崩溃刷 traceback：改为静默中止
...
- 设置页「风格配置」重做为可视化卡片：展示习惯画像状态（已学习/未学习）与习惯摘要；“立即学习/重新学习”按钮始终可见，并提供“清除”按钮

### 后端新增/变更

- `POST /api/styles/learn` 返回学习到的画像摘要与文档数
- 新增 `GET /api/styles/habit`（查询习惯画像状态+可读摘要）
- 新增 `DELETE /api/styles/habit`（清除已学习画像）
